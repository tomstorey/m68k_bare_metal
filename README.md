...
There are some different "build environment flavours" that you can choose from:
 ...
 - **pie_application**: this type of build uses a Global Offset Table to make it Position Independently Executable. The GOT is a table of pointers to variables and functions. When the application wants to call a function or access a variable it loads its pointer from the GOT and can then call or access that item using a pointer. By using pointers it is therefore possible to have applications and data that are bigger than 32K. Register A5 (usually) is set up as the GOT pointer, and GOT entries are accessed as an offset from the value of A5. With each pointer consuming 4 bytes, and relative operations being limited to +/- 32K, the GOT is therefore limited to approximately 8192 entries. This kind of build can also be useful for device drivers, or larger/more complex applications that require more than 32K of code and data.
...
The crt0 included with the application builds is generally very simple. Just clearing the BSS and maybe copying some initialised data to RAM. Other considerations are required for 
things like the stack pointer, and what happens if the application returns. If you're using these as a basis for application development in some kind of OS that you are building, 
you'll probably want to rework these crt0's to cater for these things, and potentially more (argv and argc for example). Consider the crt0's as a starting point, rather than a fully fleshed out implementation.
